...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark justify-content-between">
    ...
    <?php
        ...
        $sql = "SELECT directorFName, directorSName FROM director WHERE directorID = '$m'";
        $result = $conn->query($sql);
        $row =$result->fetch_assoc();
        $val1 = $row['directorFName'];
        $val2 = $row['directorSName'];
        $sql1 = "SELECT * FROM director WHERE directorFName = '$val1' AND directorSName = '$val2'";
        $sql2 = "SELECT movie.movieID, movie.name, movie.year FROM director INNER JOIN movie ON director.directorID = movie.directorID WHERE directorFName = '$val1' AND directorSName = '$val2'";
    ...
    <tr>
        ...
        while(($row1 =$result1->fetch_assoc())) {
            $m2 = $row1["directorID"];
        ?>
        <td><h3><?= $row1["directorFName"];?> <?= $row1["directorSName"];?></h3>
        <a href="edit_director.php?mVar=<?= $m2;?>"> +Edit This Record</a><br>
        Date of Birth: <?= $row1["directorDOB"];?>
...
<a href="delete_director.php?mVar=<?= $m2;?>"> +Delete This Record</a>
</body>

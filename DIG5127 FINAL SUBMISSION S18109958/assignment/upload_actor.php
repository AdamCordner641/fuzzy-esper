...
<?php
    if (isset($_POST['upload'])) {
        ...
        $actorImg  = $_FILES['actorImg']['name'];
        $target = "actors/".$actorImg;
        if (move_uploaded_file($_FILES['actorImg']['tmp_name'], 'actors/'.$_FILES['actorImg']['name']) === TRUE) {
            $sql = "INSERT INTO actor(actorFName, actorSName, actorDOB, actorImg) VALUES ('$actorFName','$actorSName','$actorDOB','$actorImg')";
            ...
            ?>
            <center><h3>Failed to upload file.</h3></center>
            <?php
            }
        } else  {
                ?>

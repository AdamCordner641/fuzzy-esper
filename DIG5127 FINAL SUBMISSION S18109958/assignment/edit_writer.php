...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark justify-content-between">
    ...
    <?php
        ...
        $val = $m;
        $sql1 = "SELECT * FROM writer WHERE writerID = '$val'";
    ...
    <table>
        <tr>
            <form action="update_writer.php" method="post" enctype="multipart/form-data">
            <?php
            while(($row1 =$result1->fetch_assoc())) {
                $m2 = $row1["writerID"];
            ?>
                <input type="hidden" name="writerID" value="<?= $row1["writerID"];?>">
                <td><h3><b>Name:</b> <input pattern='[^"]+' required type="text" name="writerFName" value="<?= $row1["writerFName"];?>"> <input pattern='[^"]+' required type="text" name="writerSName" value="<?= $row1["writerSName"];?>"></h3>
                <b>DoB:</b><br><input value="<?= $row1["writerDOB"];?>" required type="date" name="writerDOB"> <br>

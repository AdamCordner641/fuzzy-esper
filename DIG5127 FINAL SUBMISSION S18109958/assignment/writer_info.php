...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark justify-content-between">
    ...
    <?php
        ...
        $sql = "SELECT writerFName, writerSName FROM writer WHERE writerID='$m'";
        $result = $conn->query($sql);
        $row =$result->fetch_assoc();
        $val1 = $row["writerFName"];
        $val2 = $row["writerSName"];
        $sql1 = "SELECT * FROM writer WHERE writerFName = '$val1' AND writerSName = '$val2'";
        $sql2 = "SELECT movie.movieID, movie.name, movie.year FROM writer INNER JOIN movie_writer ON writer.writerID = movie_writer.writerID INNER JOIN movie ON movie_writer.movieID = movie.movieID WHERE writerFName = '$val1' AND writerSName = '$val2'";
    ...
    <tr>
        ...
        while(($row1 =$result1->fetch_assoc())) {
            $m2 = $row1["writerID"];
        ?>
        <td><h3><?= $row1["writerFName"];?> <?= $row1["writerSName"];?></h3>
        <a href="edit_writer.php?mVar=<?= $m2;?>"> +Edit This Record</a><br>
        Date of Birth: <?= $row1["writerDOB"];?>
        <br><b>Projects: </b><br>
        <?php
        }
...
<a href="delete_writer.php?mVar=<?= $m2;?>"> +Delete This Record</a>
</body>

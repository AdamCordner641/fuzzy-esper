...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark justify-content-between">
    ...
    <table>
        <tr>
            <form action="update_genre.php" method="post" enctype="multipart/form-data">
            <?php
            while(($row1 =$result1->fetch_assoc())) {
                $m2 = $row1["genreID"];
            ?>
                <input type="hidden" name="genreID" value="<?= $row1["genreID"];?>">
                <td><h3><b>Name:</b> <input pattern='[^-,"]+' required type="text" name="genreName" value="<?= $row1["genreName"];?>"></h3>
                <b>Desription:</b> <input required type="text" pattern='[^-"]+' name="genreDesc" value="<?= $row1["genreDesc"];?>"><br>

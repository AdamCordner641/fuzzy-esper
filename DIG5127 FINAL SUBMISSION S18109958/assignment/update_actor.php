...
<?php
    if (isset($_POST['upload'])) {
        ...
        $actorDOB = date("Y-m-d", strtotime($_POST['actorDOB']));
        $actorImg = $_FILES['fileToUpload']['name'];
            $target = "actors/".$actorImg;
        
        if ($actorImg != null) {
            $sql = 'UPDATE actor SET actorFName="'.$actorFName.'",actorSName="'.$actorSName.'", actorDOB="'.$actorDOB.'", actorImg="'.$actorImg.'" WHERE actorID="'.$actorID.'"';
            ...
            $sql = 'UPDATE actor SET actorFName="'.$actorFName.'",actorSName="'.$actorSName.'", actorDOB="'.$actorDOB.'" WHERE actorID="'.$actorID.'"';
            if ($conn->query($sql) === TRUE) {
                ?>
                <center>
                    <h3>Record updated successfully.</h3><br>

...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark justify-content-between">
    ...
    <?php
        ...
        $sql = "SELECT studioName FROM studio WHERE studioID='$m'";
        $result = $conn->query($sql);
        $row =$result->fetch_assoc();
        $val = $row["studioName"];
        $sql1 = "SELECT * FROM studio WHERE studioName = '$val'";
        $sql2 = "SELECT movie.movieID, movie.name, movie.year FROM studio INNER JOIN movie ON studio.studioID = movie.studioID WHERE studioName = '$val'";
    ...
    <tr>
        ...
        while(($row1 =$result1->fetch_assoc())) {
            $m2 = $row1["studioID"];
        ?>
        <td><h3><?= $row1["studioName"];?></h3>
            <a href="edit_studio.php?mVar=<?= $m2;?>"> +Edit This Record</a><br>
            Headquarters: <?= $row1["studioAddress"];?><br>
            Founded: <?= $row1["yearFounded"];?><br>
            Founders: <?= $row1["founder"];?> <br>
            Key People: <?= $row1["people"];?> <br>
...
<a href="delete_studio.php?mVar=<?= $m2;?>"> +Delete This Record</a>
</body>

...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark justify-content-between">
    ...
    <?php
        ...
        $val = $m;
        $sql1 = "SELECT * FROM director WHERE directorID = '$val'";
    ...
    <table>
        <tr>
            <form action="update_director.php" method="post" enctype="multipart/form-data">
            <?php
            while(($row1 =$result1->fetch_assoc())) {
                $m2 = $row1["directorID"];
            ?>
                <input type="hidden" name="directorID" value="<?= $row1["directorID"];?>">
                <td><h3><b>Name:</b> <input pattern='[^"]+' required type="text" name="directorFName" value="<?= $row1["directorFName"];?>"> <input pattern='[^"]+' required type="text" name="directorSName" value="<?= $row1["directorSName"];?>"></h3>
                <b>DoB:</b><br><input value="<?= $row1["directorDOB"];?>" required type="date" name="directorDOB"> <br>
